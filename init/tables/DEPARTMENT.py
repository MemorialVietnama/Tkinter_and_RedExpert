...
from function.fetch_data import fetch_department_data
...
from function.translate_colums import translate_dep_columns
from button_function.add_button import open_add_dep_window
from button_function.refresh_button import refresh_dep_table, color_rows,sort_column
from button_function.delete_button import delete_selected_dep_employer
from tables.button_function.edit_button import open_edit_dep_window
from tables.button_function.search_button import open_search_dep_window
...
def main():
    ...
    root.title(f"Таблица Отделов - Авторизованный пользователь: {user_name} {first_name} {middle_name} {last_name}")
    ...
    print("Checking TKINTER okay...")
    conn = connect_to_database(username, password)
    if conn is None:
        return
    print("Checking Connection okay...")
    ...
    if role == "Admin": 
        ttkb.Button(button_frame, bootstyle="primary", text="Добавить запись", command=lambda: open_add_dep_window (conn,tree)).pack(side=LEFT, padx=5)
        ttkb.Button(button_frame, bootstyle="primary", text="Удалить запись", command=lambda: delete_selected_dep_employer (conn, tree, columns)).pack(side=LEFT, padx=5)
        ttkb.Button(button_frame, bootstyle="primary", text="Изменить запись", command=lambda: open_edit_dep_window (conn, tree, columns)).pack(side=LEFT, padx=5)
    ttkb.Button(button_frame, bootstyle="primary", text="Найти запись", command=lambda: open_search_dep_window (conn, tree, columns)).pack(side=LEFT, padx=5)
    ttkb.Button(button_frame, bootstyle="primary", text="Обновить Таблицу", command=lambda: refresh_dep_table (conn, tree)).pack(side=LEFT, padx=5)
