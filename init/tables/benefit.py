...
from function.fetch_data import fetch_benefits_data
...
from function.translate_colums import translate_bef_columns
from button_function.add_button import open_add_benefit_window
from button_function.refresh_button import refresh_bef_table, color_rows,sort_column
from button_function.delete_button import delete_selected_benefits
from tables.button_function.edit_button import open_edit_benefit_window
from tables.button_function.search_button import open_search_bef_window
...
def main():
    ...
    root.title(f"Таблица Льгот - Авторизованный пользователь: {user_name} {first_name} {middle_name} {last_name}")
    root.geometry(f"{root.winfo_screenwidth()}x{root.winfo_screenheight()}")
    root.state('zoomed') 
    current_dir = Path(__file__).resolve().parent
    icon_path = current_dir.parents[1] / 'src' / 'r_app.ico'
    root.iconbitmap(icon_path)
    print("Check tkinter ok")
    ...
    if role == 'Admin' :
        ttkb.Button(button_frame, bootstyle="primary", text="Добавить запись", command=lambda: open_add_benefit_window(conn,tree)).pack(side=LEFT, padx=5)
        ttkb.Button(button_frame, bootstyle="primary", text="Удалить запись", command=lambda:delete_selected_benefits(conn, tree, columns)).pack(side=LEFT, padx=5)
        ttkb.Button(button_frame, bootstyle="primary", text="Изменить запись", command=lambda: open_edit_benefit_window(conn, tree, columns)).pack(side=LEFT, padx=5)
    ttkb.Button(button_frame, bootstyle="primary", text="Найти запись", command=lambda: open_search_bef_window(conn, columns,tree)).pack(side=LEFT, padx=5)
    ttkb.Button(button_frame, bootstyle="primary", text="Обновить Таблицу", command=lambda: refresh_bef_table(conn, tree)).pack(side=LEFT, padx=5)
