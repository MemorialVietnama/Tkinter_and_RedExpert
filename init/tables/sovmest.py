...
from function.fetch_data import fetch_sovmest_data
...
from function.translate_colums import translate_sovm_columns
from button_function.add_button import open_add_sovm_window
from button_function.refresh_button import refresh_sovm_table, color_rows,sort_column
from button_function.delete_button import delete_selected_sovm_post
from tables.button_function.edit_button import open_edit_sovmest_window
from tables.button_function.search_button import open_search_sovm_window
...
def main():
    ...
    root.title(f"Таблица Совместительство - Авторизованный пользователь: {user_name} {first_name} {middle_name} {last_name}")
    ...
    if role == "Admin":
        ttkb.Button(button_frame, bootstyle="primary", text="Добавить совместительство", command=lambda: open_add_sovm_window (conn, tree)).pack(side=LEFT, padx=5)
        ttkb.Button(button_frame, bootstyle="primary", text="Удалить совместительство", command=lambda: delete_selected_sovm_post (conn, tree, columns)).pack(side=LEFT, padx=5)
        ttkb.Button(button_frame, bootstyle="primary", text="Изменить совместительство", command=lambda: open_edit_sovmest_window (conn, tree, columns)).pack(side=LEFT, padx=5)
    ttkb.Button(button_frame, bootstyle="primary", text="Найти совместительство", command=lambda: open_search_sovm_window (conn, tree, columns)).pack(side=LEFT, padx=5)
    ttkb.Button(button_frame, bootstyle="primary", text="Обновить Таблицу", command=lambda: refresh_sovm_table (conn, tree)).pack(side=LEFT, padx=5)
    ...
    def set_column_widths(tree, columns):
        ...
        for col in columns:
            ...
            tree.column(col, anchor='center', width=max_width)  # Добавляем небольшой запас
